...
try:    
    ...
    while y < rows:
        bpy.ops.object.duplicate_move(TRANSFORM_OT_translate={"value":(0, 7.5, 0)})
        y += 1
    ...
    while y == rows and x < columns:
        bpy.ops.object.duplicate_move(TRANSFORM_OT_translate={"value":(-2.5, 0, 0)})
        x += 1
    ...
    for i in ("Pyramid", "Pyramid.002", "Pyramid.004", "Pyramid.006", "Pyramid.008", "Pyramid.010"):
        ...
        material.diffuse_color = (255, 0, 0, 1) # Red
    ...
    for i in ("Pyramid.001", "Pyramid.003", "Pyramid.005", "Pyramid.007", "Pyramid.009", "Pyramid.011"):
        ...
        if object: obj.select_set(True)
        # Add screw modifier
        modifier = object.modifiers.new('Screw', 'SCREW')
        modifier.screw_offset = 2
        # Add material
        material = bpy.data.materials.new(name='Lime') 
        material.diffuse_color = (0, 255, 0, 1) # Lime
        object.data.materials.append(material)
    ...
    for i in ("Raised Hexagon.001", "Raised Hexagon.003", "Raised Hexagon.005", "Raised Hexagon.007", "Raised Hexagon.009", "Raised Hexagon.011"):
        ...
        if object: obj.select_set(True)
        # Add remesh modifier
        modifier = object.modifiers.new('Remesh', 'REMESH')
        # Add material
        material = bpy.data.materials.new(name='Yellow') 
        material.diffuse_color = (255, 255, 0, 1) # Yellow
        object.data.materials.append(material)
    ...
    for i in ("Raised Pyramid", "Raised Pyramid.002", "Raised Pyramid.004", "Raised Pyramid.006", "Raised Pyramid.008", "Raised Pyramid.010"):
        ...
        if object: obj.select_set(True)
        # Add bevel modifier
        modifier = object.modifiers.new('Bevel', 'BEVEL')
        # Add material
        material = bpy.data.materials.new(name='Cyan') 
        material.diffuse_color = (0, 255, 255, 1) # Cyan
        object.data.materials.append(material) 
    ...
    for i in ("Raised Pyramid.001", "Raised Pyramid.003", "Raised Pyramid.005", "Raised Pyramid.007", "Raised Pyramid.009", "Raised Pyramid.011"):
        ...
        # modifier.screw_offset = 2
        # Add material
        material = bpy.data.materials.new(name='Magenta') 
        material.diffuse_color = (255, 0, 255, 1) # Magenta
        object.data.materials.append(material)
